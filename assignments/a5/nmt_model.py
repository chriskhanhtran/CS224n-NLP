...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)    # (src_len, b, max_word_length)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)    # (tgt_len, b, max_word_length)
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device)   # Tensor: (tgt_len, b)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden = torch.cat((last_hidden[0, :], last_hidden[1, :]), axis=1)    # (2, b, h) -> (b, h*2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decoder(Ybar_t, dec_state)    # shape (b, h)
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2)
            # Input: dec_hidden: (b, h), enc_hiddens_proj: (b, src_len, h); Target: (b, src_len)
