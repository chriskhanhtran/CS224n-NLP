...
def pad_sents_char(sents, char_pad_token):
    ...
    # Pad words:
    # Trim words longer than 21 characters
    sents_padded = [[w[:max_word_length] + [char_pad_token] * (max_word_length - len(w)) for w in s] for s in sents]
...
def pad_sents(sents, pad_token):
    ...
    max_sent_length = max(len(s) for s in sents)
    sents_padded = [s + [pad_token]*(max_sent_length - len(s)) for s in sents]
